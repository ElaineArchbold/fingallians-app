...
const LOGO = "data:image/png;base64,..."; // Fingallians crest
const CHALLENGE_LOGO = "data:image/png;base64,..."; // Challenge logo
...
export default function App() {
  ...
  async function loadAllPlayers() {
    const { data } = await sb.from("players").select("id,name,squad").eq("squad", APP_SQUAD).order("name");
    setAllPlayers(data || []);
  }
...
function LinkPlayerScreen({ onLink }) {
  ...
  useEffect(() => {
    sb.from("players").select("id,name,squad").eq("squad", APP_SQUAD).order("name").then(({ data }) => {
      setPlayers(data || []);
      setLoading(false);
    });
  }, []);
...
function ScoresTab() {
  ...
  useEffect(() => {
    async function load() {
      const { data: players } = await sb.from("players").select("id,name,squad").eq("squad", APP_SQUAD);
...
function DashboardTab({ allPlayers }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      sb.from("audit_log").select("user_email,player_name,action,detail,created_at,squad").eq("squad", APP_SQUAD).order("created_at",{ascending:false}).limit(20),
