...
const APP_BUILD_NOTE = "2026-07-01 bonus approval persistence + Friday Night Hurling";
    ...
    if (isChildView) {
      if (isSquadSession) {
        ...
        if (nextComplete) {
          ...
          showToast(`🏅 ${bonusApprovalLabel(taskKey)} submitted! Ask a parent to share proof in WhatsApp. Points appear once approved.`);
        ...
        return (
          <div className="squad-card" style={{background:"linear-gradient(135deg,#0b5d1e,#063d14)",borderColor:"rgba(255,255,255,0.18)"}}>
            <div className="squad-hd" onClick={()=>{
              ...
              <div className="squad-hd-text">
                <div className="squad-type">
                  Friday Night Hurling · +{PTS.fridayHurling} pts {pending ? "· Awaiting approval" : returned ? "· Returned" : ""}
                </div>
                <div className="squad-name">Fridays at 6pm · Main Pitch, Fingallians</div>
              </div>
            ...
            {expandedFridayHurling && (
              <div className="squad-body">
                <p className="squad-desc">Attend Friday Night Hurling on Friday at 6pm on the Main Pitch in Fingallians. This is an extra bonus activity and points appear once approved.</p>
                <div className="squad-cta">🏅 Attended Friday Night Hurling? Ask a parent to send photo or video proof in WhatsApp. Points appear once approved.</div>
                {returned && (
                  <div style={{background:"#ffebee",border:"1px solid #ffcdd2",color:"#c62828",borderRadius:10,padding:"10px 12px",fontSize:13,fontWeight:800,marginBottom:12}}>
                    ↺ Returned by admin — please check WhatsApp/proof and submit again.
                  </div>
...
function ProgressTab({ player, checks, isAdmin, allPlayers = [] }) {
  ...
  const weeklyData = useMemo(() => {
    return WEEKS.map(w => {
      ...
      if (checks[squadKey(w.week)] || checks[fridayHurlingKey(w.week)]) squad = 1;
...
function DashboardTab({ allPlayers, onRefresh, showToast }) {
  ...
  async function reviewBonus(row, decision) {
    ...
    if (error || !updatedRows?.length) {
      console.error("Bonus review failed", error || "No matching bonus row was updated", row);
