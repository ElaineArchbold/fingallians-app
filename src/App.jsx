...
//   3. Run the SQL schema (see bottom of this file) in your Supabase SQL editor
//   4. Deploy to Vercel / Netlify — it's a single JSX file, just drop it in a
//      Vite + React project alongside an index.html
//
// ADMIN ACCESS:
//   Set ADMIN_EMAIL below to your email. Logging in with that email shows the
...
// ── STATIC WEEK PLAN (tasks + YouTube video IDs per skill) ───────────────────
// youtube_id: the part after ?v= in a YouTube URL. Replace with real GAA drill
// videos before launch. Current values are high-quality GAA coaching demos.
const WEEKS = [
  {
  ...
  },
  {
    ...
    skills:[
      {id:"h2a",label:"🏑 Ground Ball Drill",desc:"20 mins: ground ball, first touch and lift immediately, flick to the wall. Build your speed and reactions.",youtube_id:"DemoYTID07"},
      {id:"f2a",label:"⚽ Solo Run Drill",desc:"Solo the ball 30m without dropping it. Try both feet. Count how many clean solos in a row — record your best!",youtube_id:"DemoYTID05"},
    ],
  ...
  },
  {
    ...
    skills:[
      {id:"h3a",label:"🏑 Free Taking – 21m",desc:"15 hurling frees from the 21. Count how many go over. Record your score — coaches will retest in September!",youtube_id:"DemoYTID08"},
      {id:"f3a",label:"⚽ Scoring from the D",desc:"10 shots from different spots around the D. Count your score. Can you get 7+ out of 10?",youtube_id:"DemoYTID11"},
    ],
    squad:{label:"Squad Session – Free Taking Competition",desc:"Two rounds: (1) Hurling: each lad takes 5 frees from the 21, add up the group total. (2) Football: each lad takes 5 shots from the D, add group total. Record both — coaches want to see these scores again in September!",youtube_id:"DemoYTID09"},
  },
  {
    week:4, phase:"Building", dates:"Jul 20–26",
    runs:[{label:"Run 1",distance:"2.5k"},{label:"Run 2",distance:"3k"},{label:"Run 3",distance:"3k"}],
    skills:[
      {id:"h4a",label:"🏑 Ball Wall – Combination",desc:"25 mins: overhead, ground and close-range striking mixed. No stopping to reset between touches.",youtube_id:"DemoYTID13"},
      {id:"f4a",label:"⚽ Hand-Pass & Kick Combo",desc:"25 mins: fist-pass to wall, catch, kick-pass. Mix both feet and hands. Keep moving the whole time.",youtube_id:"DemoYTID10"},
    ],
    squad:{label:"Squad Session – Score Hunt",desc:"Set up near any goals. (1) Hurling: each lad takes 5 pucks from 20m — group total? (2) Football: each lad takes 5 shots from around the D — group total? Target: 15+ out of 20 for each. Keep the scores!",youtube_id:"DemoYTID12"},
  },
  {
    week:5, phase:"Push", dates:"Jul 27–Aug 2",
    runs:[{label:"Run 1",distance:"3k"},{label:"Run 2",distance:"3k"},{label:"Run 3",distance:"3.5k"}],
    skills:[
      {id:"h5a",label:"🏑 45-Metre Free",desc:"5 attempts from the 45m line. Score out of 5. Repeat on the other side. Coaches will test this in September!",youtube_id:"DemoYTID14"},
      {id:"f5a",label:"⚽ Solo & Score",desc:"Timed drill: solo 40m, turn, shoot. Record your time. Try to beat it by end of the week.",youtube_id:"DemoYTID17"},
    ],
    squad:{label:"Squad Session – 45 & Score Chase",desc:"Push week challenge. (1) Hurling: each lad takes 5 attempts from the 45 — group total from both sides. (2) Football: 3-man weave, pass and follow up the pitch, finish with a score. How many scores in 5 mins? Record everything.",youtube_id:"DemoYTID15"},
  },
  {
    week:6, phase:"Push", dates:"Aug 3–9",
    runs:[{label:"Run 1",distance:"3k"},{label:"Run 2",distance:"3.5k"},{label:"Run 3",distance:"3.5k"}],
    skills:[
      {id:"h6a",label:"🏑 Ball Wall – Match Speed",desc:"30 mins at full pace. Strike, receive, strike again. No slow touches. Do this AFTER a run for extra challenge.",youtube_id:"DemoYTID19"},
      {id:"f6a",label:"⚽ 3-Man Weave Drill",desc:"3 lads across the pitch, pass and follow your pass. Weave to goal, finish with a score. How many in 5 mins?",youtube_id:"DemoYTID18"},
    ],
    squad:{label:"Squad Session – Puck-Around & Weave",desc:"Two mini-games. (1) Hurling: 2v2 near the goal, one side attacks for 3 mins, count scores, swap. (2) Football: 3-man weave timed challenge — how many scores in 5 mins as a group? Record both.",youtube_id:"DemoYTID21"},
  },
  {
    week:7, phase:"Peak", dates:"Aug 10–16",
    runs:[{label:"Run 1",distance:"3.5k"},{label:"Run 2",distance:"3.5k"},{label:"Run 3",distance:"4k"}],
    skills:[
      {id:"h7a",label:"🏑 Consecutive Frees",desc:"Most accurate hurling frees from 20m in a row without a miss. Personal best — beat it!",youtube_id:"DemoYTID20"},
      {id:"f7a",label:"⚽ Points from Both Feet",desc:"10 shots — 5 from left, 5 from right foot, 13m out. Count your score. Can you get 7+ out of 10?",youtube_id:"DemoYTID11"},
    ],
    squad:{label:"Squad Session – Peak Challenge",desc:"Full session both codes. (1) Hurling: each lad does 10 frees from 20m — group total. Compare to Week 3! (2) Football: each lad does 10 shots, 5 each foot — group total. Compare to Week 3! How much has the team improved?",youtube_id:"DemoYTID21"},
  },
  {
    week:8, phase:"Peak", dates:"Aug 17–23",
    runs:[{label:"Run 1",distance:"3.5k"},{label:"Run 2",distance:"4k"},{label:"Run 3",distance:"4k"}],
    skills:[
      {id:"h8a",label:"🏑 Final Ball Wall Session",desc:"35 mins hurling — full pace, both sides, ground and overhead. Leave nothing in the tank. This is your last session before assessment!",youtube_id:"DemoYTID19"},
      {id:"f8a",label:"⚽ Final Solo & Kick Test",desc:"Solo the ball 30m clean, both feet. Compare to your Week 2 score. Then kick for score — compare to Week 3. How far have you come?",youtube_id:"DemoYTID05"},
    ],
    squad:{label:"Squad Session – Final Challenge",desc:"Re-run the Week 2 squad session: (1) Hurling ground ball relay — how many clean rounds in 5 mins? (2) Football kick-pass — how many clean takes as a group? Compare BOTH scores to Week 2. Screenshot and send to the coaches! 📸",youtube_id:"DemoYTID06"},
  },
];
...
// ── TASK KEY HELPERS ──────────────────────────────────────────────────────────
// Task keys: `w{week}-run{n}`, `w{week}-skill-{id}`, `w{week}-squad`
...
// ── CSS ───────────────────────────────────────────────────────────────────────
const CSS = `
@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@600;700;800;900&family=Source+Serif+4:wght@400;600;700&family=Lato:wght@400;700;900&display=swap');
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --g:#a31621; --g2:#c41e2e; --g3:#fde8ea;
  --cream:#fffdf9; --white:#ffffff;
...
/* ── Shell ── */
.shell{max-width:480px;margin:0 auto;padding-bottom:88px;position:relative}

/* ── Header ── */
.hdr{
  background:var(--g);padding:18px 18px 0;
...
.crest{
  ...
  box-shadow:0 0 0 2px var(--gold),0 0 0 4px rgba(255,255,255,0.15);
  background:white;
}
...
.hdr-sub{font-size:11px;color:rgba(255,255,255,0.65);margin-top:2px;font-family:'Lato',sans-serif}
.hdr-player{font-size:12px;color:var(--gold2);font-weight:700;margin-top:2px}
.tabs{display:flex;gap:3px}
.tab-btn{
  flex:1;padding:10px 2px;border:none;border-radius:10px 10px 0 0;
  font-family:'Barlow Condensed',sans-serif;font-size:14px;letter-spacing:0.04em;
  cursor:pointer;background:rgba(255,255,255,0.12);color:rgba(255,255,255,0.65);
...
.auth-hero{
  background:var(--g);border-radius:var(--radius);padding:30px 24px 26px;
  margin-bottom:18px;color:white;position:relative;overflow:hidden;text-align:center;
}
...
.btn-gold:hover{background:#b88a10}
...
.wk-tile{
  background:var(--card);border-radius:12px;padding:10px 6px;text-align:center;
  box-shadow:var(--shadow);cursor:pointer;transition:all 0.15s;border:2px solid transparent
}
...
.skill-check{
  width:28px;height:28px;border-radius:50%;border:2px solid #e0c0c0;
...
.yt-placeholder{
  ...
  background:linear-gradient(135deg,#4a0a0e 0%,#1a0405 100%);color:white;
...
/* Squad card special styling */
.squad-card{
  background:linear-gradient(135deg,#7d1018 0%,var(--g) 100%);
  border-radius:var(--radius);box-shadow:var(--shadow-lg);margin-bottom:12px;overflow:hidden;
}
...
.player-email{font-size:12px;color:var(--muted)}
...
.toast{
  ...
  background:var(--g);color:white;padding:10px 22px;border-radius:24px;
  font-weight:700;font-size:14px;box-shadow:0 6px 28px rgba(163,22,33,0.35);
...
// (shown as a comment — copy from here to deploy)
/*
...
create policy "Players visible to all authenticated users"
  on players for select using (auth.role() = 'authenticated');

create policy "parent_players: own rows only"
  on parent_players for all using (user_id = auth.uid());

create policy "completions: linked players only"
  on task_completions for all using (
    player_id in (
      select player_id from parent_players where user_id = auth.uid()
    )
  );
*/
// ─────────────────────────────────────────────────────────────────────────────
...
export default function App() {
  ...
  const [player, setPlayer]     = useState(null); // linked player for this parent
  const [checks, setChecks]     = useState({});   // { task_key: true }
  const [allPlayers, setAllPlayers] = useState([]); // admin only
  ...
  // Load player + completions once logged in
  useEffect(() => {
  ...
  async function toggleTask(taskKey, pts, label) {
    ...
    if (done) {
      await sb.from("task_completions")
        .delete().eq("player_id", player.id).eq("task_key", taskKey);
          ...
          <div className="hdr">
            <div className="hdr-row">
              ...
              <div>
                ...
                <div className="hdr-sub">2014 Boys · Summer Challenge 2026</div>
...
function HomeTab({ player, checks, pts, weeksDone, onNav, onToggle }) {
  ...
  // current week (rough: based on July 1 start)
  useEffect(() => {
      ...
      <div className="welcome-card">
        ...
        <div className="pts-row">
          <div className="pts-box">
            ...
            <div className="lbl2">Total Points</div>
          </div>
          <div className="pts-box">
            <div className="num">{weeksDone}</div>
            <div className="lbl2">Weeks Done</div>
          </div>
          <div className="pts-box">
            <div className="num">{8-weeksDone}</div>
            <div className="lbl2">Weeks Left</div>
          </div>
        </div>
      </div>
...
// ══════════════════════════════════════════════════════════════════════════════
// WEEK DETAIL (shared between Home and Plan tabs)
// ══════════════════════════════════════════════════════════════════════════════
              ...
              <div className="skill-body">
                ...
                {canToggle && (
                  <button className={`mark-btn${done?" mark-undone":" mark-done"}`}
                    onClick={()=>onToggle(k,PTS.skill,s.label)}>
          ...
          <div className="squad-card">
            ...
            {expandedSquad && (
              <div className="squad-body">
                ...
                {canToggle && (
                  <button className={`squad-mark${done?" done":""}`}
                    onClick={()=>onToggle(k,PTS.squad,w.squad.label)}>
...
// ── YouTube embed with lazy-load placeholder ──────────────────────────────────
function VideoEmbed({ ytId, playing, onPlay, dark }) {
  // If ytId is a placeholder, show a styled placeholder
      ...
      {(isPlaceholder || !playing) ? (
        <div className="yt-placeholder" style={dark?{background:"rgba(0,0,0,0.35)"}:{}} onClick={!isPlaceholder?onPlay:undefined}>
          ...
          <div className="yt-note">
            {isPlaceholder
              ? "Video demo coming soon\nAdmin: add a YouTube ID in the WEEKS data"
              : "Tap to watch drill demonstration"}
          </div>
        </div>
      ) : (
        <iframe
...
// ══════════════════════════════════════════════════════════════════════════════
// PLAN TAB — scrollable all-weeks view
// ══════════════════════════════════════════════════════════════════════════════
...
function AdminTab({ allPlayers, onRefresh, showToast }) {
  ...
  const [playerStats, setPlayerStats] = useState({}); // player_id -> {pts, checks}

  // Load all completions for all players
  useEffect(() => {
    async function load() {
  ...
  return (
    <div className="admin-wrap">
      ...
      <div className="add-form">
        ...
        <input className="inp" placeholder="e.g. Ciarán Murphy" value={newName} onChange={e=>setNewName(e.target.value)}
          onKeyDown={e=>e.key==="Enter"&&addPlayer()} />
        <button className="btn btn-green btn-sm" onClick={addPlayer} disabled={adding}>
          {adding?"…":"+ ADD TO SQUAD"}
        </button>
      </div>
      ...
      <div className="card" style={{marginTop:20}}>
        ...
        <div className="card-bd" style={{fontSize:13,color:"var(--mid)",lineHeight:1.7}}>
          To add real drill videos, edit the <code style={{background:"#f0f0f0",padding:"1px 5px",borderRadius:4}}>WEEKS</code> array at the top of this file.<br/><br/>
          For each skill, find a YouTube video of the drill and copy the video ID (the part after <code style={{background:"#f0f0f0",padding:"1px 5px",borderRadius:4}}>?v=</code> in the URL).<br/><br/>
          Replace <code style={{background:"#f0f0f0",padding:"1px 5px",borderRadius:4}}>DemoYTID01</code> etc. with the real IDs.
          <div style={{marginTop:10,padding:"10px 12px",background:"#f0f7f1",borderRadius:8}}>
            e.g. <code>youtube.com/watch?v=<strong>dQw4w9WgXcQ</strong></code><br/>
            → use <code><strong>dQw4w9WgXcQ</strong></code>
          </div>
        </div>
      </div>
    </div>
